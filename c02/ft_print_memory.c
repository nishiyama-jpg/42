...
//converts a number into hexadecimal and prints it.
...
void	ft_print_addr(unsigned long long addr) //Converts a number into hexadecimal and prints it.
{
	unsigned long long	tmp;
	int					i;

	// This temporary variable is created so as not to change the value of ADDR
	tmp = addr;//to count two digits without destroying the original address.
	i = 1;
